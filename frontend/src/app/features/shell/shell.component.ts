...
const NAV = [
  { id: 'home', label: 'Home', path: '/home', icon: 'M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z M9 22V12h6v10' },
  { id: 'explore', label: 'Explorar', path: '/explore', icon: 'M21 21l-4.35-4.35 M11 11m-8 0a8 8 0 1 0 16 0a8 8 0 0 0 -16 0' },
  { id: 'schedule', label: 'Agenda', path: '/schedule', icon: 'M3 4h18v18H3z M16 2v4 M8 2v4 M3 10h18', requireAuth: true },
  { id: 'messages', label: 'Mensagens', path: '/messages', icon: 'M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z', badge: '3', requireAuth: true },
  { id: 'provider', label: 'Prestador', path: '/provider', icon: 'M2 7h20v14H2z M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16', badgePro: true, requireAuth: true },
];
        ...
        <aside style="width:220px;flex-shrink:0;height:100vh;position:sticky;top:0;
          ...
          @for (item of visibleNav; track item.id) {
            <a [routerLink]="item.path"
              ...
              {{ item.label }}
              @if (item.badge) {
                <span style="margin-left:auto;background:var(--re);color:white;width:17px;height:17px;
          ...
          <div style="margin-top:auto">
            @if (auth.isLoggedIn()) {
              ...
              <button class="btn btn-g" style="width:100%;margin-top:6px;font-size:12px" (click)="auth.logout()">Sair</button>
            } @else {
              <a routerLink="/auth/login" class="btn btn-p" style="width:100%;font-size:13px;text-decoration:none;justify-content:center">Entrar</a>
            }
          </div>
        </aside>
      }
        ...
        @if (mobile()) {
          <div style="position:sticky;top:0;z-index:40;background:var(--ca);
            ...
            @if (auth.isLoggedIn()) {
              ...
              <a routerLink="/auth/login" class="btn btn-p" style="font-size:12px;padding:6px 12px;text-decoration:none">Entrar</a>
            }
          </div>
        }

        <div style="flex:1;overflow-y:auto;padding:22px 26px;max-width:720px;margin:0 auto;width:100%"
